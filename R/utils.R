...
check_regions_bigwig_seqlevels <- function(x, y) {
  ...
  if (!seqlevels_all_within(x, y)) {
    if (!seqlevels_valid(x)) {
      stop(
        x_name,
        " uses seqnames that do not have a compatible entry for the species supported by Seqname. ",
        "See genomeStyles() for supported species/styles."
      )
    }
    if (!seqlevels_style(x, y)) {
      stop(
        "Mismatched seqlevelStyles detected bewteen [", x_name, "] and [", y_name, "]. ",
    ...
    if (!all(seqlevels(x) %in% seqlevels(y))) {
      stop("The seqname(s) ", missing_levels, " in [", x_name, "] are missing in [", y_name, "]")
    }
  }
}
